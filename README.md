...
- **数据同步与备份**
  - 设置页新增数据同步入口
  - 支持无密码导出完整备份，覆盖聊天、消息、角色、设置与 API key
  - 支持通过系统文件选择器手动导出到本地 `.json` 文件，以及从本地 `.json` 文件恢复
  - 数据备份页顶部支持在“本地备份”和“WebDAV 同步”两张卡片之间切换
  - WebDAV 配置通过弹窗管理，不在页面主体常驻显示
  - 支持 WebDAV 云端全量备份上传、下载与备份列表查看
  - 遇到云端冲突时不会自动合并，用户可手动选择覆盖云端，或先下载云端备份再确认是否恢复到本地
...
3. 通过顶部搜索入口检索历史消息
4. 通过角色管理页维护角色、归档和恢复
5. 通过设置页的数据同步入口执行本地备份、恢复和 WebDAV 云同步
